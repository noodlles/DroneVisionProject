...
from keras_frcnn_1.RoiPoolingConv import RoiPoolingConv
from keras_frcnn_1.FixedBatchNormalization import FixedBatchNormalization
...
def nn_base(input_tensor=None, trainable=False):
    ...
    x = Convolution2D(32, (7, 7), strides=(2, 2), name='conv1', trainable = trainable)(x)
    ...
    x = identity_block(x, 3, [32, 32, 128], stage=2, block='c', trainable = trainable)

    x = conv_block(x, 3, [64, 64, 256], stage=3, block='a', trainable = trainable)
    x = identity_block(x, 3, [64, 64, 256], stage=3, block='b', trainable = trainable)
    x = identity_block(x, 3, [64, 64, 256], stage=3, block='c', trainable = trainable)
    x = identity_block(x, 3, [64, 64, 256], stage=3, block='d', trainable = trainable)
